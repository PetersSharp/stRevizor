...
namespace RevizorUI
{
    public partial class RevizorClient : Form
    {
        ...
        private void SetSetupError(object ctrl, string txterr)
        {
            if (ctrl == null)
            {
                return;
            }
            flatThreadSafe.Run(this, (Action)(() => this.EPMain.SetIconPadding(((FlatTextBox)ctrl), 4)));
            ...
            if (this.SFDMain.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    switch (type)
                    {
                        case ExportFileType.Ip:
                            {
                                this.rv.SaveToFileIpList(this.rv.IpList, Properties.Settings.Default.setSaveFile);
                                break;
                            }
                        case ExportFileType.Url:
                            {
                                this.rv.SaveToFileIpList(this.rv.UrlList, Properties.Settings.Default.setSaveFile);
                                break;
                            }
                    }
                }
